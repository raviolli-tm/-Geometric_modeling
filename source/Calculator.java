...
public class Calculator {
	...
	static double[] CalculatePositionP(double[] ViewFrom, double[] ViewTo, double x, double y, double z) {
		...
		if (Screen.isPerspectiveMode) {
			projP = getProj(ViewFrom, ViewTo, x, y, z, P);
			double[] drawP = getDrawP(projP[0], projP[1], projP[2]);
			return drawP;
		} else{

			return getProj(x, y,z,Screen.OX, Screen.OY,Screen.step_x, Screen.step_y, Screen.step_z);
	...
	static double[] getProj(double x, double y, double z, int OX, int OY, double step_x, double step_y, double step_z){
		if(Screen.isFrontView){
			double newX =OX+((step_x) / (Screen.x2 - Screen.x1))*Toolkit.getDefaultToolkit().getScreenSize().getWidth()*(x/step_x);
			double newY =OY-((step_z) / (Screen.z2 - Screen.z1))* Toolkit.getDefaultToolkit().getScreenSize().getHeight()*(z/step_z);
			return new double[]{newX, newY};
		}
		if(Screen.isTopView) {
			double newX =OX+((step_x) / (Screen.x2 - Screen.x1))*Toolkit.getDefaultToolkit().getScreenSize().getWidth()*(x/step_x);
			double newY =OY-((step_y) / (Screen.y2 - Screen.y1))* Toolkit.getDefaultToolkit().getScreenSize().getHeight()*(y/step_y);
			return new double[]{newX, newY};
		}
		if(Screen.isSideView){

			double newX =OX+((step_z) / (Screen.z2 - Screen.z1))*Toolkit.getDefaultToolkit().getScreenSize().getWidth()*(z/step_z);
			double newY =OY-((step_y) / (Screen.y2 - Screen.y1))* Toolkit.getDefaultToolkit().getScreenSize().getHeight()*(y/step_y);
			return new double[]{newX, newY};
		}
